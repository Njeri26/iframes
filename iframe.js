...
let currentIframeUrl = sessionStorage.getItem('iframeUrl') || 'https://checheafrica.com/';
let isIframeOpen = sessionStorage.getItem('isIframeOpen') === 'true';

// Function to handle user authentication
async function authenticateUser() {
    const hostToken = localStorage.getItem('hostToken');
    const iframeId = localStorage.getItem('iframeUserId');
    
    if (!hostToken || !iframeId) {
        // Show registration modal if no tokens exist
        document.getElementById('registrationModal').style.display = 'flex';
        return null;
    }
    
    // Proceed with authentication using tokens
    return await validateTokens(hostToken, iframeId);
}
...
window.addEventListener('message', function (event) {
    if (event.origin === "https://checheafrica.com") {
